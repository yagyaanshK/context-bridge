...
Turntrail should remain easy to inspect, easy to run locally, and conservative about user data.
...
homes, editor profile, extensions, or Turntrail account store.
...
- Treat `.turntrail/` as private user data.
...
- credentials live in `~/.turntrail/`, never in a project

If you are adding a provider, prefer delegating sign-in to its official CLI. Performing the OAuth
exchange in Turntrail is a last resort, taken for Claude only because its CLI cannot be driven
