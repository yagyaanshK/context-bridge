...
1. Update the version in the root, core, CLI, and VS Code package manifests, including internal
   `@context-bridge/core` dependency versions.
...
5. Create and push the matching annotated tag, such as `v0.11.0`.
...
gh attestation verify context-bridge-<version>.vsix --repo yagyaanshK/context-bridge
```
